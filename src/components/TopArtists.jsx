...
const TopArtists = () => {
  ...
  return artistData && (
     <section className='xl:w-[28rem] p-2 mt-5 animate-left'>
          ...
          <div ref={scrollableDivRef4} onMouseMove={(e)=>dragDiv(e,scrollableDivRef4)} onMouseDown={()=>dispatch(setDragging(true))}  className={`flex gap-5  mt-4 h-40 xl:h-40  items-start overflow-x-scroll hide-scrollbar select-none ${dragging ? "cursor-grab" : "cursor-default"}`}>
            {
              artistData?.map((item,index)=>(
                <div key={index} className=' shrink-0 w-20 h-20 xl:w-30 xl:h-30 group '>
                  ...
                  <p className='text-white text-sm xl:text-base text-center mt-2 xl:translate-y-[-1.25rem] xl:opacity-0 xl:group-hover:translate-y-[0] xl:group-hover:opacity-100 transition-all duration-500 ease-in-out'>{item?.name}</p>
                </div>
              ))
            }
          </div>
