...
const ArtistDetails = () => {
  ...
  return searchResults10Data || artistData ? (
    <>
      <div className="mt-6 mb-4 xl:mt-0 xl:ml-0 sm:ml-[12rem] flex items-center gap-4 pl-4 xl:absolute xl:top-20 xl:left-55">
        <IoIosArrowRoundBack
      ...
      <section className="hide-scrollbar xl:ml-0 sm:ml-[12rem] animate-left overflow-y-scroll px-2 pb-10 xl:absolute xl:top-35  xl:left-55 xl:h-[calc(100vh_-_12rem)] xl:w-[82%]">
        <div className="flex pl-2 items-center gap-5">
          <div className="w-full max-w-30 lg:max-w-fit rounded-full text-white lg:w-35 lg:h-35 xl:h-40 xl:w-40">
            <img
              draggable="false"
              src={artistData.picture_xl}
              alt={artistData.name}
              className="rounded-full border-4 border-white object-cover lg:w-35 lg:h-35 xl:h-40 xl:w-40"
            />
          </div>

          <div>
            <h1 className="mb-2 text-lg font-bold lg:text-xl text-white xl:text-3xl">
              {artistData.name}
            </h1>
            <h2 className="mb-2 text-white xl:text-xl">
              Number of <br className="block sm:hidden" /> albums{" "}
        ...
        <div className="mt-15">
          <div className="mb-8 flex pl-2 items-center justify-between">
            <h1 className="text-xl w-50 sm:w-80 md:w-100 xl:w-full font-bold text-white xl:text-3xl">
          ...
          <div >
            {searchResults10Data?.map((item, index) => (
              <div
                key={index}
                className={`mt-2 flex items-center justify-between rounded-lg px-2 py-2 text-white xl:px-4 xl:pr-8 lg:hover:bg-[#4b456e] ${item.id === currentSongID && isPlaying ? "bg-[#4b456e]" : ""}`}
              >
                <div className="flex items-center gap-4">
                  <p>{index + 1}.</p>
                  <img
                    className="w-12 rounded-lg xl:w-15 lg:w-14"
                    src={`https://e-cdns-images.dzcdn.net/images/cover/${item?.md5_image}/1000x1000.jpg`}
                    alt="topCharts"
                  />
                  <div>
                    <Link to={`/song/${item.id}`}>
                      <h2 className="truncate text-sm lg:text-base w-35 sm:w-65  md:w-90 lg:w-150 font-bold xl:text-lg">
                        {item?.title}
                      </h2>
                    </Link>
                    <Link to={`/artist/${item.artist.id}`}>
                      <h3 className="truncate text-xs w-35 sm:w-65 md:w-90 lg:w-150  text-gray-300 lg:text-sm">
                        {item?.artist?.name}
                      </h3>
                    </Link>
                  </div>
        ...
        <div className="mt-15">
          ...
          <div >
            {uniqueAlbums?.map((item, index) => (
              <div
                key={index}
                className={`mt-2 flex items-center justify-between rounded-lg  px-2 py-2 text-white xl:px-4 xl:pr-8 lg:hover:bg-[#4b456e]`}
              >
                <div className="flex items-center gap-4">
                  <p>{index + 1}.</p>
                  <img
                  ...
                  <div>
                    <Link to={`/album/${item?.album?.id}`}>
                      <h2 className="truncate text-sm w-50 sm:w-75 lg:w-150 lg:text-base md:w-90 font-bold xl:text-lg">
                        {item?.album?.title}
                      </h2>
                    </Link>
                    <Link to={`/artist/${item.artist.id}`}>
                      <h3 className="truncate text-xs w-35 sm:w-75 lg:w-150 md:w-90 text-gray-300 lg:text-sm">
                        {item?.artist?.name}
                      </h3>
                    </Link>
                  </div>
